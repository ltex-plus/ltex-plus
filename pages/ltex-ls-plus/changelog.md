...
- &#x1f41b; *Bug fix:* Fix a race between `textDocument/didChange` and `textDocument/completion` that caused completion to return matches for a stale, pre-edit prefix. `applyTextChangeEvents` was queued on the single-thread executor while completion ran synchronously on the dispatcher, so a completion request arriving after a keystroke could observe document state from before the edit. The bug was masked in clients that debounce completion or trigger it manually (VS Code defaults) but became deterministic under aggressive auto-completion (zero delay, single-character prefix). Apply text changes synchronously on the dispatcher and capture `(text, position)` under a scoped lock in the completion path so concurrent diagnostics work continues without blocking reads. &#x2014; [Andrea Alberti (@alberti42)](https://github.com/alberti42)
