...
Die unten gezeigten modernen, vollständig ausgeschriebenen Codes erfordern ltex-ls-plus 18.7.x oder neuer (siehe [ltex-ls-plus#150](https://github.com/ltex-plus/ltex-ls-plus/pull/150)). Frühere Versionen akzeptierten nur die veralteten nackten Codes wie `"fr"`, `"it"`, `"de"` oder `"en"`; eine bestmögliche Beschreibung, wie Sprachcodes in jenen Versionen behandelt wurden, finden Sie unter [Language Codes in Older Versions](language-code-legacy.html). Aktuelle Versionen akzeptieren diese veralteten Codes weiterhin aus Gründen der Rückwärtskompatibilität, sie werden unten jedoch nicht mehr beworben und sollten in neuen Konfigurationen nicht verwendet werden.
